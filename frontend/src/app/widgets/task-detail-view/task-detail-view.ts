...
export class TaskDetailView implements OnInit, OnDestroy {
    ...
    taskEditFormModel: WritableSignal<TaskEditModel> = signal<TaskEditModel>({
        taskId: 0,
        title: '',
        body: '',
    });
    ...
    onBodyChange(content: Record<string, unknown> | string): void {
        const renderedHtml =
            typeof content === 'string' ? content : toHTML(content, this.editor.schema);
